...
> **Un solo servicio local.** El dominio de tickets + `PcInfo` **se migró desde el antiguo servicio Node
> `apireslocal`** (que escuchaba en el puerto 4000) a esta API .NET, bajo el controlador
> `ImpresionTicket`. El front ahora consume todo en `:7249`.
...
### Nuevo en v1.26.0914
Todo esto existe para una pantalla nueva de Elzyra: **POS → Configuración de estaciones**, donde el
campo "Puerto de la impresora fiscal" dejó de escribirse de memoria y hay un botón que abre la
**información del equipo**. La API es la que contesta qué hay realmente conectado.

- **`GET /api/ImpresionTicket/PuertoFiscal`**: deduce **en qué COM está la impresora fiscal**.
  **No abre ningún puerto** — ver "Cómo se detecta la impresora fiscal". Devuelve `puerto` e
  `impresoraFiscal` solo cuando la deducción es concluyente; si no, los deja en `null` y entrega
  `candidatos` para que alguien elija. Es mejor ofrecer tres opciones que inventar una.
- **`PUT /api/ImpresionTicket/ImpresoraPredeterminada`**: cambia la **impresora predeterminada de la
  máquina**, para no tener que ir a "Impresoras y escáneres" de Windows. Es la misma que resuelve
  `ImprimirAuto`, así que **decide a dónde salen los tickets**.
  > La predeterminada de Windows es **por usuario** (vive en `HKCU`), así que esto cambia la del
  > usuario bajo el que corre este proceso. En las estaciones la API se instala como **tarea
  > programada del cajero** (no como servicio, que correría como administrador y no vería la
  > predeterminada del cajero), así que es el contexto correcto.
- **`PcInfo` suma tres claves**: `PuertoFiscal` (el COM, o `null`), `ImpresoraFiscal` (el modelo, o
  `null`) y `PuertoFiscalDetalle` (el resultado completo de la detección). Son **claves nuevas**: no
  rompen a ningún consumidor anterior. Un `PcInfo` **sin** ellas significa que esa caja tiene una
  versión previa del servicio — distinto de que no haya impresora.
- **Configuración `PuertoFiscal:Modelos`** en `appsettings.json`: qué modelos se saben reconocer y
  con qué. Agregar un modelo es agregar una entrada, sin recompilar.

### Nuevo en v1.26.0903
- **`POST /api/ImpresionTicket/ImprimirAuto`**: el front manda solo el ESC/POS y **el servicio resuelve
  la impresora predeterminada** del equipo, decidiendo red (TCP 9100) vs USB. El front ya no necesita
  consumir `PcInfo` para imprimir. `Imprimir`/`ImprimirIP` quedan por compatibilidad.
- **`GET /api/ImpresionTicket/Equipo`**: devuelve solo `{ equipo: <hostname> }`. El modal de cobro lo usa
  para armar `nombre_equipo` sin traerse el `PcInfo` completo.
  > ⚠️ `nombre_equipo` **decide a qué caja entra la plata** (SP `pELZ_GenerarMovCaja`). El backend y el
  > SP quedaron intactos a propósito. Ver `docs/migracion-apireslocal-v1.26.0903.md` §7.1.

### Nuevo en v1.26.0825
- **Controlador de tickets no fiscales** (`ImpresionTicketController`, `api/ImpresionTicket`): imprime
  ESC/POS a impresoras **USB** (por nombre) o de **red** (TCP 9100), lista impresoras y expone `PcInfo`.
  Reemplaza al servicio Node `apireslocal`.
- **`PrinterService`**: capa de servicio para impresión TCP/USB, detección de impresoras de Windows,
  información de red y detección de lenguaje de la impresora.
- **Impresión de nota de crédito**: nuevo endpoint `POST /api/Impresion/ImprimirNcr`.
- **Devolución del correlativo fiscal (`numFiscal`)**: `imprimirFact` e `ImprimirNcr` ahora leen el
  **frame de estado S1** de la impresora y devuelven el número fiscal real, para que el sistema lo
  escriba de vuelta en Profit. (Antes la API imprimía pero no devolvía el correlativo.)
- **Manejo de IGTF**: cuando el documento lleva IGTF se imprimen las líneas informativas de ley y se
  envía el **comando `199`** para finalizar el documento (paridad con el `impfis` legacy).
- **Nuevos campos en el payload**: `igtf`, `forma_pagos`, `fact_num` (factura); `serial`,
  `puerto_fiscal`, `igtf`, `tipo_imp`, `porc_desc` (nota de crédito).
- **Endpoints de diagnóstico**: `Ping`, `checkDll`, `EstadoDetallado/{puerto}`, `conectar/{puerto}`.
- **Endpoints de prueba** (aislar el flujo sin cerrar caja): `PruebaFact`, `PruebaNcr`.

### Corregido
- **La impresora predeterminada se lee con `GetDefaultPrinter`, no con WMI.** `Win32_Printer.Default`
  **queda pegado**: medido el 2026-09-10, tras cambiar la predeterminada el registro
  (`HKCU\…\Windows\Device`) ya decía la nueva y WMI seguía informando la anterior, **desde un proceso
  nuevo**. Afectaba a `ObtenerImpresoraPredeterminadaWindows`, o sea a `PcInfo` y —lo importante— a
  **`ImprimirAuto`**: un cajero que cambiaba su impresora en Windows podía seguir viendo los tickets
  salir por la anterior. Ahora el nombre sale de `winspool.drv` (el mismo subsistema que lo escribe) y
  `/printers` calcula su bandera `Default` comparando contra él.
- **Número de reporte Z**: ahora se lee del **frame S1** (`SUBSTR(61,4)`), en lugar de la lógica anterior
  basada en una expresión regular sobre el estado entero, que no era confiable.
- **`PcInfo`**: las claves de la respuesta (`Equipo`, `IpResultante`, `impresoras`) se serializan con un
  `Dictionary` para **preservar el casing exacto** que espera el front (igual que el servicio Node
  original); el camelCase por defecto las rompía.
- **Puerto fiscal**: se toma de `Cabecera.PuertoFiscal`; se agregó validación de serial y fecha.
- **`fact_num`**: manejo del número de factura afectada en la nota de crédito.
